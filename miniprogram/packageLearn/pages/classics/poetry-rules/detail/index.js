...
Page({
  ...
  onLoad: function(options) {
    this._module = 'classics-poetry-rules'
  ...
  loadData: function() {
    var result = classicsData.loadModuleData(this._module)
  ...
  markAsLearned: function() {
    ...
    classicsData.markAsLearned(this._module, item.id, item.title)
  ...
  speakContent: function() {
    var item = this.data.item
    if (!item) return
    var text = item.title + '。结构：' + item.structure + '。' + item.example
    speak.speak(text)
  },
